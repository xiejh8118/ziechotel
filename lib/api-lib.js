...
function token(expiresAt = Date.now() + 8 * 60 * 60 * 1000) {
...
function authed(req) {
  ...
  const value = c.split(";").map((x) => x.trim()).find((x) => x.startsWith("ziec_admin="))?.slice(11);
...
function rateLimit(req, res, scope, limit = 20, windowMs = 60_000) {
  ...
  res.status(429).json({ ok: false, message: "请求过于频繁，请稍后再试", code: "RATE_LIMITED" });
  return false;
}
